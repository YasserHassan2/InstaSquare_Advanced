...
public class QueShotGrid {
    ...
    public QueShotGrid(Drawable drawable2, QueShotArea area2, Matrix matrix2) {
    ...
    private void animateTranslate(final View view, final float f, final float f2) {
        ...
        this.animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                QueShotGrid.this.translate(f * ((Float) valueAnimator.getAnimatedValue()).floatValue(), f2 * ((Float) valueAnimator.getAnimatedValue()).floatValue());
                view.invalidate();
            }
        });
        ...
        if (!isFilledArea()) {
            ...
            this.animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                public void onAnimationUpdate(ValueAnimator valueAnimator) {
                    ...
                    QueShotGrid.this.zoom(f, f, pointF);
                    QueShotGrid.this.postTranslate(f2, f3);
                    view2.invalidate();
                }
            });
            if (z) {
    ...
    public void updateWith(MotionEvent motionEvent, QueShotLine line) {
        ...
        if (line.direction() == QueShotLine.Direction.HORIZONTAL) {
            translate(0.0f, y);
        } else if (line.direction() == QueShotLine.Direction.VERTICAL) {
            translate(x, 0.0f);
        }
        RectF currentDrawableBounds = getCurrentDrawableBounds();
        QueShotArea area3 = getArea();
    ...
    public void setArea(QueShotArea area2) {
        this.area = area2;
    }
