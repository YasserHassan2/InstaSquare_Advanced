...
import com.que.shot.utils.MatrixUtils;
...
public class QueShotGridView extends QueShotStickerView {
    ...
    private QueShotLine quShotLine;

    public QueShotGrid queShotGrid;
    private QueShotLayout.Info info;
    private int lineColor;
    private Paint linePaint;
    private int lineSize;
    private PointF midPoint;
    private List<QueShotGrid> queShotGridList;
    ...
    public QueShotGrid prevHandlingQueShotGrid;
    private QueShotLayout queShotLayout;

    public List<QueShotGrid> queShotGrids;
    private boolean quickMode;
    private QueShotGrid replaceQuShotGrid;
    ...
    public interface onQueShotSelectedListener {
        void onQuShotSelected(QueShotGrid quShotCollage, int i);
    }
    ...
    public void onSizeChanged(int i, int i2, int i3, int i4) {
        ...
        if (this.queShotGrids.size() != 0) {
            for (int i5 = 0; i5 < this.queShotGrids.size(); i5++) {
                QueShotGrid puzzlePiece = this.queShotGrids.get(i5);
                QueShotArea area = this.queShotLayout.getArea(i5);
        ...
        if (this.queShotLayout != null) {
            ...
            if (this.info != null) {
                ...
                for (int i = 0; i < size; i++) {
                    QueShotLayout.LineInfo lineInfo = this.info.lineInfos.get(i);
                    QueShotLine line = this.queShotLayout.getLines().get(i);
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (this.queShotLayout != null) {
            ...
            while (i < this.queShotLayout.getAreaCount() && i < this.queShotGrids.size()) {
                QueShotGrid puzzlePiece = this.queShotGrids.get(i);
            ...
            if (this.needDrawOuterLine) {
                for (QueShotLine drawLine : this.queShotLayout.getOuterLines()) {
                    drawLine(canvas, drawLine);
                }
            }
            if (this.needDrawLine) {
                for (QueShotLine drawLine2 : this.queShotLayout.getLines()) {
    ...
    private void drawSelectedArea(Canvas canvas, QueShotGrid quShotCollage) {
        QueShotArea quShotCollageArea = quShotCollage.getArea();
        canvas.drawPath(quShotCollageArea.getAreaPath(), this.selectedAreaPaint);
        for (QueShotLine next : quShotCollageArea.getLines()) {
    ...
    private void drawLine(Canvas canvas, QueShotLine line) {
        canvas.drawLine(line.startPoint().x, line.startPoint().y, line.endPoint().x, line.endPoint().y, this.linePaint);
    }

    public void updateLayout(QueShotLayout quShotLayout) {
        ArrayList<QueShotGrid> arrayList = new ArrayList<>(this.queShotGrids);
        setQueShotLayout(quShotLayout);
        for (QueShotGrid drawable : arrayList) {
            addQuShotCollage(drawable.getDrawable());
        }
        invalidate();
    }

    public void setQueShotLayout(QueShotLayout quShotLayout) {
        clearQuShot();
    ...
    public void setCollageLayout(QueShotLayout.Info info) {
        this.info = info;
        clearQuShot();
        this.queShotLayout = QueShotLayoutParser.parse(info);
    ...
    public QueShotLayout getQueShotLayout() {
    ...
    private void decideActionMode(MotionEvent motionEvent) {
        for (QueShotGrid isAnimateRunning : this.queShotGrids) {
    ...
    private void prepareAction(MotionEvent motionEvent) {
        switch (this.actionMode) {
            ...
            case MOVE:
                ...
                for (QueShotGrid next : this.queShotGridList) {
                    next.record();
    ...
    private void performAction(MotionEvent motionEvent) {
        switch (this.actionMode) {
            case DRAG:
                ...
                return;
            case ZOOM:
                zoomPiece(this.queShotGrid, motionEvent);
                return;
            case MOVE:
    ...
    public void setPrevHandlingQueShotGrid(QueShotGrid puzzlePiece) {
    ...
    private void moveLine(QueShotLine line, MotionEvent motionEvent) {
        boolean z;
        if (line != null && motionEvent != null) {
            if (line.direction() == QueShotLine.Direction.HORIZONTAL) {
    ...
    private void updatePiecesInArea(QueShotLine line, MotionEvent motionEvent) {
        for (int i = 0; i < this.queShotGridList.size(); i++) {
            this.queShotGridList.get(i).updateWith(motionEvent, line);
        }
    }

    private void zoomPiece(QueShotGrid puzzlePiece, MotionEvent motionEvent) {
    ...
    private void dragPiece(QueShotGrid puzzlePiece, MotionEvent motionEvent) {
    ...
    public void setQueShotGrid(QueShotGrid puzzlePiece) {
    ...
    private QueShotGrid findHandlingQuShot() {
        for (QueShotGrid next : this.queShotGrids) {
    ...
    private QueShotLine findHandlingLine() {
        for (QueShotLine next : this.queShotLayout.getLines()) {
    ...
    private QueShotGrid findReplacePiece(MotionEvent motionEvent) {
        for (QueShotGrid next : this.queShotGrids) {
    ...
    private List<QueShotGrid> findNeedChangedPieces() {
    ...
    public void addQuShotCollage(Drawable drawable, Matrix matrix, String str) {
        ...
        QueShotArea area = this.queShotLayout.getArea(size);
        area.setPadding(this.collagePadding);
        QueShotGrid puzzlePiece = new QueShotGrid(drawable, area, new Matrix());
        if (matrix != null) {
    ...
    public QueShotGrid getQueShotGrid() {
    ...
    public void setAnimateDuration(int i) {
        this.duration = i;
        for (QueShotGrid animateDuration : this.queShotGrids) {
    ...
    public void setCollagePadding(float f) {
        ...
        if (this.queShotLayout != null) {
            ...
            int size = this.queShotGrids.size();
            for (int i = 0; i < size; i++) {
                QueShotGrid quShotCollage = this.queShotGrids.get(i);
    ...
    public List<QueShotGrid> getQueShotGrids() {
        int size = this.queShotGrids.size();
